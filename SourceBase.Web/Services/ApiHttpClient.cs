...
public class ApiHttpClient(HttpClient http, BlazorAuthStateProvider auth)
{
    ...
    private async Task<(T? data, ErrorResponse? error)> ExecuteAsync<T>(HttpRequestMessage req)
    {
        try
        {
    ...
    private async Task<ErrorResponse?> ExecuteAsync(HttpRequestMessage req)
    {
        try
        {
            var response = await http.SendAsync(req);
    ...
    public Task<(LoginResponse? data, ErrorResponse? error)> RefreshTokenAsync(string token) =>
        ExecuteAsync<LoginResponse>(Request(HttpMethod.Post, "/api/auth/refresh", new { token }));
