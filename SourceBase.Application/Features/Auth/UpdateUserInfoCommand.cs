...
public class UpdateUserInfoCommandHandler(IIdentityService identityService, IUserContext userContext) : IRequestHandler<UpdateUserInfoCommand>
{
    public async Task Handle(UpdateUserInfoCommand request, CancellationToken cancellationToken)
    {
        await identityService.UpdateUserInfoAsync(userContext.UserId, request.FirstName, request.LastName, cancellationToken);
    }
}
